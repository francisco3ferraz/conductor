...
func TestMemoryStore_Jobs(t *testing.T) {
	...
	// Create a job
	j := job.New(job.TypeImageProcessing, []byte("payload"), 5, 3, 0, "test-node")
...
func TestMemoryStore_JobFiltering(t *testing.T) {
	...
	j1 := job.New(job.TypeImageProcessing, []byte("1"), 5, 3, 0, "test-node")
	j1.Status = job.StatusPending

	j2 := job.New(job.TypeWebScraping, []byte("2"), 3, 3, 0, "test-node")
	j2.Assign("worker-1")
	j2.Status = job.StatusAssigned

	j3 := job.New(job.TypeDataAnalysis, []byte("3"), 1, 3, 0, "test-node")
...
func TestMemoryStore_Concurrent(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		go func(id int) {
			j := job.New(job.TypeImageProcessing, []byte("payload"), 1, 3, 0, "test-node")
			store.SaveJob(j)
			done <- true
		}(i)
	}
