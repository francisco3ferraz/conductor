...
func TestExecutor_Execute_ImageProcessing(t *testing.T) {
	...
	j := job.New(job.TypeImageProcessing, []byte("test-image.jpg"), 5, 3, 0, "test-node")
...
func TestExecutor_Execute_WebScraping(t *testing.T) {
	...
	j := job.New(job.TypeWebScraping, []byte("https://example.com"), 5, 3, 0, "test-node")
	ctx := context.Background()
...
func TestExecutor_Execute_DataAnalysis(t *testing.T) {
	...
	j := job.New(job.TypeDataAnalysis, []byte("data.csv"), 5, 3, 0, "test-node")
	ctx := context.Background()
